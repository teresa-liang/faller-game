# make_project5_submission.py
#
# ICS 32 Winter 2022
# Project 5
# SUBMISSION-BUILDING AUTOMATION
#
# [last updated: 2021-12-11 15:29]

from pathlib import Path
import zipfile


SUBMISSION_NAME = 'project5.zip'
...
def create_submission(submission_path: Path, script_dir_path: Path) -> bool:
    ...
    if len([file for file in files_to_submit if file.suffix == '.py']) == 0:
        ...
        print('  and run it again (from that directory).')
        return False

    any_issues = False

    for file in files_to_submit:
        if file.suffix == '.py' and not verify_format_py(file):
            any_issues = True
